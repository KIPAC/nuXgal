...
for n_evt in n_events:
    ...
    syn_cls = hp_utils.vector_cross_correlate_maps(max_map, syn_maps, Defaults.NCL)
...
o_dict = figs.plot_cl("cl", lvals, syn_cls_means, ymin=1e-20, labels=labels) #yerr=syn_cls_stds, ymin=1e-20)


testfigpath = os.path.join(Defaults.NUXGAL_PLOT_DIR, 'test')  
testfigfile = os.path.join(testfigpath, 'gal_cross_sig_v_nevt')
